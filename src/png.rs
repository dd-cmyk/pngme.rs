...
impl Png {
    // Constants associated with a type must be in the 'impl' block
    pub const STANDARD_HEADER: [u8; 8] = [137, 80, 78, 71, 13, 10, 26, 10];

    fn from_chunks(chunks: Vec<Chunk>) -> Self {
        // Do we take owneship here?
        Png { chunks }
    }
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        // Remove the first chunk with the chunk_type that we matched
        // and then return Result with Ok containing that chunk
    ...
    fn header(&self) -> &[u8; 8] {
        // This functions just returns the constant, yes?
        &Self::STANDARD_HEADER
    }
    pub fn chunks(&self) -> &[Chunk] {
        // We can use deref coercion (or something like and just return chunks)
        &self.chunks
    }
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        match self
            ...
            .position(|x| x.chunk_type().bytes() == chunk_type.as_bytes())
        {
            Some(pos) => Some(&self.chunks[pos]),
            None => None,
        }
    }
    pub fn as_bytes(&self) -> Vec<u8> {
        // Chunk type has a method called as_bytes()
        // We should just join them all
        // and we should add the header also
        let mut result: Vec<u8> = vec![];
        result.append(&mut Self::STANDARD_HEADER.to_vec());
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        // Perform a check for a valid header
        if value[0..8] != Self::STANDARD_HEADER[..] {
            return Err("ERROR".into());
        }
        //
        // The chunk type has TryFrom implemented
        // So we must just determine where
        // a chunk begins and where it ends, and pass appropriate slices to
        // the method defined in the chunk type

        // The first 8 bytes are the header, for now we don't check if the header is sound

        // 1. set start_idx = 8
        // 2. len = read the len of "Chunk Data" part
        // 3. calculate end_idx, end_idx = start_idx + 4 + 4 + len + 4
        // 4. use the try_from implementation of Chunk with the slice[start_idx..end_idx]
        // 5. set start_idx = end_idx
        // 6. repeat until start_idx is in range

        let mut data: Vec<Chunk> = vec![];

        /*
        //Just add one chunk for now
        let len: usize = u32::from_be_bytes(value[8..12].try_into().unwrap()) as usize;
        println!("{len}");
        println!("{:?}", &value[8..20+len]);
        let c: Chunk = value[8..20 + len].try_into().unwrap();
        println!("{:?}", c.as_bytes());
        data.push(c); */

        let mut start_idx: usize = 8;
        while start_idx <= value.len() - 12 {
            // We could get some out of range access here
            // but if we do, it's our fault, because
            // the tests only pass correct chunks
            let len: usize =
                u32::from_be_bytes(value[start_idx..start_idx + 4].try_into().unwrap()) as usize;
            let end_idx: usize = start_idx + 4 + 4 + len + 4;
            let c: Chunk = value[start_idx..end_idx].try_into().unwrap();
            data.push(c);
            start_idx = end_idx;
        }
        Ok(Png::from_chunks(data))
    }
}

impl std::fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Just display all the chunks
